...
class ROADS(object):
    ...
    # TODO 更新道路权重
    def GetRCostRtFtoT(self, NextRoad, cross):
        return NextRoad.CostFromtoTo + NextRoad.GetNextRoadAvailableCapacity(cross) + self.FromtoToCapcity

    def GetRCostRtTtoF(self, NextRoad, cross):
        return NextRoad.TotoFromRoad + NextRoad.GetNextRoadAvailableCapacity(cross) + self.TotoFromCapcity
        ...
        for i in range(self.Rlength):  # 从车道前面往后面开始扫描
            if RoadDataStruct[i][channel] is not None:
                ...
                if Car.state == 2:  # 当前车辆为停止状态
                    ...
                    continue
                if (i - Sv) > FrontCarPos:  # 可以行驶到停止状态 且无车阻挡
        ...
        if NextRoadDis <= 0:  # 无法通过路口
            ...
            for channle in range(NextRoad.Rchannel):
                ...
                if NextRoadPos == -1:  # 没有挡住当前车辆的行走
                    NextRoadStruct[NextRoad.Rlength - NextRoadDis][channle] = NowRoadStruct[Car.nowDistanceX][Car.nowChannleY]
                ...
                if FrontCar.state == 1:  # 前面车辆为等待状态 标记当前车辆为等待状态 不移动
                    ...
                    NextRoadStruct[NextRoadPos + 1][channle] = NowRoadStruct[Car.nowDistanceX][Car.nowChannleY]
                    NowRoadStruct[Car.nowDistanceX][Car.nowChannleY] = None  # 清空上一个位置
                    NextRoad.UpdateRoadCarsNumAdd(cross)
                    self.UpdateRoadCarsNumSub(cross)
                    Car.UpdatePosAndSta(state=2)
