...
class Graph(object):
    def __init__(self, car_path, road_path, cross_path, answer_path, OnesnumberCar):
        ...
        for i in range(len(self.rRoads.roadsInfo)):  # 扫描每一辆车，对图相应的边进行赋值4 2
            graph[int(self.rRoads.roadsInfo[i].start) - 1][int(self.rRoads.roadsInfo[i].to) - 1] = roadIdAndLength(0.5 * self.rRoads.roadsInfo[i].channel + 0.1 * self.rRoads.roadsInfo[i].speed, self.rRoads.roadsInfo[i].id)
            if (int(self.rRoads.roadsInfo[i].isDuplex)):  # 判断是否为双向道路
                graph[int(self.rRoads.roadsInfo[i].to) - 1][int(self.rRoads.roadsInfo[i].start) - 1] = roadIdAndLength(0.5 * self.rRoads.roadsInfo[i].channel + 0.1 * self.rRoads.roadsInfo[i].speed, self.rRoads.roadsInfo[i].id)# 如果为双向道路，则对应位置赋值边长（道路长度）
    ...
    # 为每一辆车初始规划最短行驶路径
    def findCarShortPathNotUpdate(self):
        for i in range(len(self.cars.carsInfo)):  # 为每一辆车规划最短路径
            distance, path = self.dijkstra(int(self.cars.carsInfo[i].start) - 1, int(self.cars.carsInfo[i].to) - 1)  # 得到车辆的路径信息（路口到路口）
            #self.updateGraph(path)
            #print(self.cars.carsInfo[i].id, distance, path)
